...
def set_budget(request):
    ...
    for cat_name_raw, data in budget_data_map.items():
        ...
        data["display_name"] = category_display_map.get(
            cat_name.lower(), cat_name.replace("_", " ").title()
        )  # Use display map for consistency

        data["remaining"] = remaining
        data["progress_percentage"] = min(
            100, round(progress_percentage, 2)
        )  # Cap at 100%
